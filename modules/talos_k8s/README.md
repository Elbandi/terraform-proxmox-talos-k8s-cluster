...
| <a name="input_nodes"></a> [nodes](#input\_nodes) | Configuration for worker nodes | <pre>map(object({<br/>    machine_type = string<br/>    ip           = string<br/>    swap_size    = optional(number, 0)<br/>    install_disk = optional(string, "/dev/sda")<br/>    user_disks = optional(list(object({<br/>      type = optional(string)<br/>      dev  = optional(string)<br/>      name = optional(string)<br/>    })), [])<br/>    extra_mounts = optional(list(object({<br/>      destination = string<br/>      type        = string<br/>      source      = string<br/>      options     = optional(list(string), [])<br/>    })), [])<br/>    gpu            = optional(string)<br/>    time_server    = optional(string)<br/>    kernel_modules = optional(list(string), [])<br/>    node_labels    = optional(map(any), {})<br/>    custom_network = optional(string)<br/>  }))</pre> | n/a | yes |
